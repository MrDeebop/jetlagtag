...
function showGoalSetup() {
  ...
  for (const player of state.players) {
    ...
    card.innerHTML = `
      ...
      <label>COORDINATES
        <input class="goal-coords" type="text" placeholder="e.g. 40.7° N, 74.0° W" autocomplete="off" autocorrect="off" spellcheck="false" />
      </label>
      <p class="goal-coords-feedback hint"></p>
    ...
    const coordInput = card.querySelector(".goal-coords");
    const feedback   = card.querySelector(".goal-coords-feedback");
    coordInput.addEventListener("input", () => {
      const parsed = parseCoords(coordInput.value);
      if (coordInput.value.trim() === "") {
        feedback.textContent = "";
        feedback.className = "goal-coords-feedback hint";
      } else if (parsed) {
        feedback.textContent = `✓ ${parsed.lat.toFixed(5)}, ${parsed.lng.toFixed(5)}`;
        feedback.className = "goal-coords-feedback hint coords-ok";
      } else {
        feedback.textContent = "✗ format not recognised — try: 40.7° N, 74.0° W";
        feedback.className = "goal-coords-feedback hint coords-err";
      }
    });
...
// Accepts formats emitted by Google Maps long-press, e.g.:
//   40.7° N, 74.0° W      (degree symbol optional)
//   40.7128 N, 74.0060 W
//   40.7128° N 74.0060° W  (comma optional)
//   -40.7128, 74.0060      (plain decimal, sign encodes hemisphere)
// Returns { lat, lng } or null if unparseable.
function parseCoords(raw) {
  if (!raw) return null;
  const s = raw.trim();

  // ── Pattern 1: directional — e.g. "40.7128° N, 74.0060° W" ──────────────
  const dir = s.match(
  ...
  if (dir) {
    ...
    if (/S/i.test(dir[2])) lat = -Math.abs(lat);
    if (/W/i.test(dir[4])) lng = -Math.abs(lng);
    if (lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) return { lat, lng };
  }

  // ── Pattern 2: plain decimal pair — e.g. "40.7128, -74.0060" ─────────────
...
async function submitAllGoals() {
  ...
  for (const card of cards) {
    ...
    const coordsRaw = card.querySelector(".goal-coords").value;
    const parsed    = parseCoords(coordsRaw);
    const label     = card.querySelector(".goal-label").value.trim();
    const player    = state.players.find((p) => p.id === playerId);

    if (!parsed) {
      toast(`Enter valid coordinates for ${player?.name}. Try pasting from Google Maps.`, "danger"); return;
    }
    if (!label) {
      toast(`Enter a hint for ${player?.name}.`, "danger"); return;
    }
    goals.push({ playerId, lat: parsed.lat, lng: parsed.lng, label });
  }
