...
// ═══════════════════════════════════════════════════════════════════════════
//
// Runner view:  own dot (blue) · goal pins · chaser ping dots (purchased)
// Chaser view:  all player dots · goal pins · high-accuracy runner dot (orange)
//               + tracker-off blackout overlay
...
function renderUnifiedMap() {
  ...
  if (titleEl) titleEl.textContent = isChaser ? "📡 LIVE MAP" : "🗺 MAP";
...
function updatePlayerDotsOnMap(locations) {
  ...
  for (const [pid, loc] of Object.entries(locations)) {
    ...
    // Runners on the runner's own view: just show self as blue dot.
    // Chasers' view: show all players.
    // Runner's view: show chasers only if ping was purchased (those arrive via showChaserPingOnMap).
    //   However, we do show self (runner) as own dot always.
    const meIsRunner  = me?.role === "runner";
    const meIsChaser  = me?.role === "chaser";

    // Runner sees: only themselves (chasers shown via ping markers)
    if (meIsRunner && !isMe) continue;

    const latlng      = L.latLng(loc.lat, loc.lng);
    const displayName = (loc.playerName || "").replace(" (Gamemaster)", "");

    // Color: self = bright green, runner = orange, chaser = blue
    let color;
    if (isMe)          color = "#22cc66";
    else if (isRunner) color = "#ff5500";
    else               color = "#4da6ff";
