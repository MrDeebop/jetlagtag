...
let transportHighwayRate    = 0;    // car highway surcharge per minute
let transportHighwayMinutes = 0;    // accumulated highway minutes (from server)
let transportOnHighway      = false;
...
function buildTransportModes() {
  ...
  for (const { mode, icon, label } of TRANSPORT_MODES) {
    ...
    const highwaySurcharge = (mode === "car" && gameConstants?.TRANSPORT_RATE_CAR_HIGHWAY != null)
      ? gameConstants.TRANSPORT_RATE_CAR_HIGHWAY
      : null;
    let rateStr;
    if (rate == null) {
      rateStr = "";
      ...
      rateStr = " (Free)";
    } else if (highwaySurcharge != null) {
      rateStr = ` (${rate}/min + ${highwaySurcharge}/min highway)`;
    } else {
      rateStr = ` (${rate}/min)`;
    }
...
function renderTransportCard() {
  ...
  if (timer) {
    ...
    const baseRate = gameConstants?.TRANSPORT_RATES?.[timer.mode] ?? timer.ratePerMinute ?? 0;
    const onHighway = timer.onHighway || false;
    const highwaySurcharge = (timer.mode === "car" && gameConstants?.TRANSPORT_RATE_CAR_HIGHWAY != null)
      ? gameConstants.TRANSPORT_RATE_CAR_HIGHWAY : 0;
    const effectiveRate = baseRate + (onHighway ? highwaySurcharge : 0);

    let rateStr;
    if (effectiveRate === 0) {
      rateStr = " (Free)";
    } else if (timer.mode === "car") {
      rateStr = onHighway
        ? ` (${baseRate} + ${highwaySurcharge} HWY = ${effectiveRate}/min 🛣️)`
        : ` (${baseRate}/min)`;
    } else {
      rateStr = ` (${effectiveRate}/min)`;
    }
    document.getElementById("transport-mode-label").textContent =
      m ? `${m.icon} ${m.label}${rateStr}` : timer.mode;

    if (!transportTimerInterval && me) {
      const elapsed       = timer.elapsedSeconds || 0;
      transportTimerStart = Date.now() - elapsed * 1000;
      transportTimerRate  = baseRate;
      transportHighwayRate = highwaySurcharge;
      transportHighwayMinutes = timer.highwayMinutes || 0;
      startLocalTimer();
    } else {
      // Update highway state from server push
      transportHighwayRate    = highwaySurcharge;
      transportHighwayMinutes = timer.highwayMinutes || 0;
      transportOnHighway      = onHighway;
    }
  } else {
...
async function startTransport() {
  ...
  try {
    ...
    const baseRate  = gameConstants?.TRANSPORT_RATES?.[selectedMode] ?? 0;
    const hwyRate   = (selectedMode === "car" && gameConstants?.TRANSPORT_RATE_CAR_HIGHWAY != null)
      ? gameConstants.TRANSPORT_RATE_CAR_HIGHWAY : 0;

    transportTimerStart     = Date.now();
    transportTimerRate      = baseRate;
    transportHighwayRate    = hwyRate;
    transportHighwayMinutes = 0;
    transportOnHighway      = false;
    startLocalTimer();
    ...
    if (baseRate === 0) {
      ...
      rateStr = ` (${baseRate}/min + ${hwyRate}/min on highway)`;
    } else {
      rateStr = ` (${baseRate}/min)`;
    }
    document.getElementById("transport-mode-label").textContent =
      modeInfo ? `${modeInfo.icon} ${modeInfo.label}${rateStr}` : selectedMode;

    toast(`${modeInfo?.label ?? selectedMode} timer started${baseRate > 0 ? ` (${baseRate} 🪙/min${hwyRate > 0 ? ` + ${hwyRate} highway` : ""})` : " (free)"}`);
  } catch (err) {
    toast(err.message, "danger");
  }
}
...
function tickLocalTimer() {
  ...
  // Base cost over full elapsed time
  const baseCost   = Math.round((elapsedMs / 60000) * transportTimerRate);
  // Highway surcharge on accumulated highway minutes (from last server sync)
  const hwyExtra   = Math.round(transportHighwayMinutes * transportHighwayRate);
  const cost       = baseCost + hwyExtra;
  const mm = String(Math.floor(elapsedSec / 60)).padStart(2, "0");
  const ss = String(elapsedSec % 60).padStart(2, "0");
  document.getElementById("transport-elapsed").textContent  = `${mm}:${ss}`;
  document.getElementById("transport-est-cost").textContent =
    transportOnHighway && transportHighwayRate > 0
      ? `≈ ${cost} 🪙 🛣️`
      : `≈ ${cost} 🪙`;
}
...
function renderShopCard() {
  ...
  for (const item of getShopItemsClient()) {
    ...
    const avgNote = item.id === "double_skip" && gameConstants
      ? `<span class="shop-item-avg-note">Priced at avg challenge reward (${gameConstants.AVG_CHALLENGE_REWARD} 🪙)</span>`
      : "";
    row.innerHTML = `
...
function buildRulesModal(c) {
  ...
  // Transport rates string — car gets a special note about the highway surcharge
  const carHighway = c?.TRANSPORT_RATE_CAR_HIGHWAY;
  const transportStr = c
    ? Object.entries(c.TRANSPORT_RATES)
        .map(([mode, rate]) => {
          const label = mode.charAt(0).toUpperCase() + mode.slice(1);
          if (rate === 0) return `${label} free`;
          if (mode === "car" && carHighway != null) {
            return `Car ${rate}/min (+ ${carHighway}/min on highways 🛣️)`;
          }
          return `${label} ${rate}/min`;
        })
        .join(" · ")
    : "Walk free · Bike 6 🪙/min · Bus 15 🪙/min · Train 30 🪙/min · Taxi 45 🪙/min · Car 20 🪙/min + 15 🪙/min on highways";

  // Shop items string — double_skip gets an extra note explaining its dynamic price
  const shopStr = c
    ? c.SHOP_ITEMS.map((i) => {
        const note = i.id === "double_skip"
          ? ` <em>(= avg challenge reward)</em>`
          : "";
        return `${i.emoji} ${i.label} — ${coin(i.cost)}${note}`;
      }).join("<br>")
  ...
  const sections = [
    ...
    },
    {
      ...
      html: `You start each turn with a ${c ? coin(c.RUNNER_BONUS) : "50 🪙"} bonus (everyone starts with ${c ? coin(c.STARTING_CURRENCY) : "100 🪙"}). Your goal is shown on the Goals Map — navigate there without being tagged.
             <br><br>
             <strong>Transport</strong> costs coins per minute — only the Runner pays. ${transportStr}. The 🚗 Car has a base rate plus an <strong>automatic highway surcharge</strong> (${c && carHighway != null ? coin(carHighway) : "15 🪙"}/min extra) that kicks in whenever GPS detects you're on a motorway or highway — no manual input needed. Stop the timer when you exit or you'll keep getting charged.
             <br><br>
