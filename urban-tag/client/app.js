...
async function init() {
  ...
  // This triggers the browser's permission dialog right away so players
  // aren't caught off guard when the game actually needs their GPS.
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      () => {},          // success — permission granted, discard the fix
      (err) => {
        if (err.code === 1) {
          // PERMISSION_DENIED — warn early so they know features will be limited
          toast("📍 Location access denied. GPS features won't work.", "danger", 6000);
        }
        // POSITION_UNAVAILABLE / TIMEOUT — transient, don't alarm the user
      },
