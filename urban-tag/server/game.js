...
function vetoChallenge(state) {
  ...
  const s         = clone(state);
  s.vetoTimer     = { startedAt: Date.now() };
...
function serializeState(state) {
  ...
  if (s.vetoTimer && s.vetoTimer.startedAt) {
    s.vetoTimer = {
      remainingMs: Math.max(0, VETO_DURATION_MS - (Date.now() - s.vetoTimer.startedAt)),
    };
  }
...
module.exports = {
  TRANSPORT_RATES, CHALLENGE_POOL,
