...
// Node 18+ has global fetch; polyfill for older versions
...
// ─── Periodic highway detection for car transport ─────────────────────────────
// Every 15 s: if the runner has an active car session AND we have a GPS fix,
// call the Nominatim reverse-geocode API to check if they're on a highway.
// Highway road types: motorway, motorway_link, trunk, trunk_link.

const HIGHWAY_ROAD_TYPES = new Set(["motorway", "motorway_link", "trunk", "trunk_link"]);
const HIGHWAY_CHECK_INTERVAL_MS = 15000;

setInterval(async () => {
  if (!gameState || gameState.phase !== "running") return;
  const runner = getCurrentRunner(gameState);
  if (!runner) return;
  const timer = gameState.transportTimers[runner.id];
  if (!timer || timer.mode !== "car") return;
  if (!runnerLocation) return;

  try {
    const { lat, lng } = runnerLocation;
    const url = `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=18&addressdetails=0`;
    const resp = await fetch(url, {
      headers: { "User-Agent": "UrbanTagGame/1.0" },
      signal: AbortSignal.timeout(8000),
    });
    if (!resp.ok) return;
    const data = await resp.json();
    const roadType = data?.extratags?.highway || data?.type || "";
    const onHighway = HIGHWAY_ROAD_TYPES.has(roadType);
    gameState = updateCarHighway(gameState, runner.id, onHighway);
    broadcastState();
  } catch (_) {
    // Nominatim unreachable — skip this check, don't update highway state
  }
}, HIGHWAY_CHECK_INTERVAL_MS);
