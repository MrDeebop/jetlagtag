...
// ─── All-player location store ────────────────────────────────────────────────
// Coarse, infrequent positions for every player (shown on the unified map).
// { [playerId]: { lat, lng, accuracy, timestamp, playerName, role } }
let playerLocations = {};
...
app.post("/api/location/update", (req, res) => {
  ...
  // Also update the coarse player locations so the unified map stays fresh
  playerLocations[runner.id] = {
    lat,
    lng,
    accuracy:   typeof accuracy === "number" ? Math.round(accuracy) : null,
    timestamp:  Date.now(),
  ...
  // Broadcast player locations update
...
/**
 * POST /api/location/player
 * Body: { playerId, lat, lng, accuracy? }
 * Coarse self-location for any player (runner or chaser).
 * Called infrequently (~every 30 s) from each device.
 * Broadcasts updated locations to all clients.
 */
app.post("/api/location/player", (req, res) => {
  if (!requireGame(res)) return;
  if (gameState.phase !== "running") {
    return res.status(400).json({ error: "Game is not running." });
  }
 ...
 * Returns all known player locations (for clients joining mid-game).
 */
