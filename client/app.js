...
socket.on("transport:tick", ({ playerId, estimatedCost }) => {
  // Server pushes every 5s — update display for other players' costs if needed
  // (for this player, the local interval handles display)
});
...
async function createGame() {
  ...
  try {
    ...
    state = data.state;
    // Move to goal setup for the first runner
    showGoalSetup();
  } catch (err) {
...
async function submitGoal() {
  ...
  try {
    ...
    // First round: also start the game
    if (state.phase === "lobby") {
      await api("POST", "/api/game/start");
    }

    // state:update will arrive via socket and trigger renderAll()
...
function renderTransportCard() {
  ...
  if (timer) {
    // An active session exists (possibly resumed after page reload)
    active.classList.remove("hidden");
    document.getElementById("transport-mode-label").textContent =
      TRANSPORT_MODES.find((m) => m.mode === timer.mode)?.label || timer.mode;

    // If our local timer isn't running, start it from the elapsed seconds the server sent
...
async function tagRunner() {
  ...
  try {
    ...
    if (finished) {
      ...
      // Show goal setup for next runner (host action)
      showGoalSetup();
    }
  } catch (err) {
    toast(err.message, "danger");
...
function wireEvents() {
  ...
  // Transport
  // Mode buttons are wired in buildTransportModes()
  // Start transport: clicking any mode-btn while no timer is active starts it
  document.getElementById("transport-modes").addEventListener("click", (e) => {
    const btn = e.target.closest(".mode-btn");
    if (!btn) return;
    const mode = btn.dataset.mode;
    selectMode(mode);
    // If no active timer for this player, immediately start
    const me = myPlayer();
